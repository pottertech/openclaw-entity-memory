...
async function loadCases() {
    const filePath = path.resolve(process.cwd(), "tests/fixtures/promotion-cases.json");
    const raw = await fs.readFile(filePath, "utf8");
    return JSON.parse(raw);
}
async function main() {
    const entityBaseUrl = process.env.ENTITY_MEMORY_BASE_URL ?? "http://localhost:4017";
    const semanticBaseUrl = process.env.SEMANTIC_BASELINE_BASE_URL ?? "http://localhost:4020";
    ...
    for (const caseDef of cases) {
        try {
            ...
            rows.push({
                ...
                semanticCorrect: result.semantic.answer === caseDef.expectedAnswer,
