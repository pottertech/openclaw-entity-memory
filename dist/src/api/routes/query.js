...
export function createQueryRouter(entityService, traversalService, hybridQueryService, impactQueryService, queryAuditService) {
    ...
    router.post("/query/path", async (req, res) => {
        ...
        try {
            ...
            const path = await traversalService.findPathByResolvedIds(tenantId, fromXid, toXid, parsed.data.maxDepth ?? 4, parsed.data.asOf);
            const responseJson = {
                found: Boolean(path),
                path: (path ?? []).map((hop) => ({
                    from: hop.from,
    ...
    router.post("/query/hybrid", async (req, res) => {
        const started = Date.now();
