...
export function createShadowReportRouter(pool) {
    const router = createRouter();
    router.get("/shadow-report/verdicts", async (req, res) => {
    ...
    router.get("/shadow-report/thresholds", async (req, res) => {
        const tenantId = String(req.query.tenant_id ?? "").trim();
      ...
      `, [tenantId, queryClass]);
        ...
        const meetsInitialPromotionThreshold = total >= 10 && hybridPreferenceRate >= 0.6 && sameAnswerRate >= 0.8;
        res.json({
            queryClass,
            total,
            preferredHybrid,
