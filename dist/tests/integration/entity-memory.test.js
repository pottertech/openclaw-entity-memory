...
describe("entity memory core", () => {
    it("resolves Alice as a known entity", async () => {
        ...
        const result = await pool.query(`SELECT xid, canonical_name FROM entities
       WHERE canonical_name ILIKE $1 AND tenant_id = $2`, ["%alice%", TEST_TENANT]);
    ...
    it("resolves Project Atlas as a known entity", async () => {
        ...
        const result = await pool.query(`SELECT xid, canonical_name FROM entities
       WHERE canonical_name ILIKE $1 AND tenant_id = $2`, ["%atlas%", TEST_TENANT]);
        expect(result.rows.length).toBeGreaterThan(0);
    });
    it("resolves PostgreSQL Cluster as a known entity", async () => {
        ...
        const result = await pool.query(`SELECT xid, canonical_name FROM entities
       WHERE canonical_name ILIKE $1 AND tenant_id = $2`, ["%postgresql%", TEST_TENANT]);
        expect(result.rows.length).toBeGreaterThan(0);
    });
    it("resolves Tuesday Outage as a known entity", async () => {
        ...
        const result = await pool.query(`SELECT xid, canonical_name FROM entities
       WHERE canonical_name ILIKE $1 AND tenant_id = $2`, ["%tuesday%", TEST_TENANT]);
        expect(result.rows.length).toBeGreaterThan(0);
    });
    it("has edges for Alice leads Project Atlas", async () => {
        ...
        const result = await pool.query(`SELECT e.xid FROM edges e
       ...
       WHERE f.canonical_name ILIKE $1
         ...
         AND e.tenant_id = $3`, ["%alice%", "%atlas%", TEST_TENANT]);
        expect(result.rows.length).toBeGreaterThan(0);
    });
    it("has edges for Project Atlas depends on PostgreSQL Cluster", async () => {
       ...
       WHERE f.canonical_name ILIKE $1
         AND t.canonical_name ILIKE $2
         AND e.tenant_id = $3`, ["%atlas%", "%postgresql%", TEST_TENANT]);
        expect(result.rows.length).toBeGreaterThan(0);
    });
    it("has edges for PostgreSQL Cluster affected by Tuesday Outage", async () => {
       ...
       WHERE f.canonical_name ILIKE $1
         AND t.canonical_name ILIKE $2
         AND e.tenant_id = $3`, ["%postgresql%", "%tuesday%", TEST_TENANT]);
        expect(result.rows.length).toBeGreaterThan(0);
    });
    ...
    it("writes a query_audit record when a query is run", async () => {
        ...
        const before = await pool.query(`SELECT COUNT(*)::int AS cnt FROM query_audit WHERE tenant_id = $1`, [TEST_TENANT]);
        const beforeCount = Number(before.rows[0].cnt);
        await pool.query(`INSERT INTO query_audit (xid, tenant_id, question, query_type, duration_ms, request_json, response_json)
       VALUES ($1, $2, $3, $4, $5, '{}'::jsonb, $6)`, [
        ...
        const after = await pool.query(`SELECT COUNT(*)::int AS cnt FROM query_audit WHERE tenant_id = $1`, [TEST_TENANT]);
    ...
    it("ACL binding exists for test tenant", async () => {
        ...
        const result = await pool.query(`SELECT COUNT(*)::int AS cnt FROM acl_bindings WHERE tenant_id = $1`, [TEST_TENANT]);
