...
export function createCanaryDashboardRouter(pool: pg.Pool): Router {
  const router = createRouter();

  router.get(
    "/canary-dashboard/outage-impact",
    async (req: Request, res: Response) => {
      ...
      const shadow = shadowSummary.rows[0] as any;
      const audit = auditSummary.rows[0] as any;
      const threshold = thresholdSummary.rows[0] as any;
      ...
      res.json({
        ...
        exclusions: exclusionSummary.rows.map((row: any) => ({
          reason: row.reason,
        ...
        thresholds: {
          ...
          hybridPreferenceRate:
            total > 0 ? Math.round((preferredHybrid / total) * 100) / 100 : 0,
          sameAnswerRate:
            total > 0 ? Math.round((sameAnswerCount / total) * 100) / 100 : 0,
        },
      });
    },
  );
...
export function createCanaryDashboardAlertsRouter(pool: pg.Pool): Router {
  const router = createRouter();

  router.get(
    "/canary-dashboard/outage-impact/alerts",
    async (req: Request, res: Response) => {
      const tenantId = String(req.query.tenant_id ?? "").trim();
      ...
      const row: any = thresholdResponse.rows[0] ?? {
        total: 0,
        preferred_hybrid: 0,
        same_answer_count: 0,
      };
      ...
      const hybridPreferenceRate =
        total > 0 ? Number(row.preferred_hybrid) / total : 0;
      const sameAnswerRate =
        total > 0 ? Number(row.same_answer_count) / total : 0;

      const alerts: Array<{ level: string; reason: string }> = [];
      ...
      const exclusionRows = exclusionResponse.rows.map((item: any) => ({
        reason: item.reason,
        count: Number(item.count),
      }));


      const missingEvidence = exclusionRows.find(
        (item) => item.reason === "missing_evidence",
      );
