export type ScoredPathHop = {
  edgeXid: string;
  edgeType: string;
  from: string;
  to: string;
  edgeAuthorityRank: number;
  evidenceVisibleCount: number;
  evidenceHiddenCount: number;
  confidence: number;
};
...
export type ScoredPath = {
  ...
  averageAuthorityRank: number;
  averageConfidence: number;
};

export class PathScorer {
  scorePath(hops: ScoredPathHop[]): ScoredPath {
    if (hops.length === 0) {
      return {
        hops,
        totalScore: 0,
    ...
    const visibleEvidenceCount = hops.reduce(
      (sum, hop) => sum + hop.evidenceVisibleCount,
      0,
    );

    const hiddenEvidenceCount = hops.reduce(
      (sum, hop) => sum + hop.evidenceHiddenCount,
      0,
    );

    const averageAuthorityRank =
      hops.reduce((sum, hop) => sum + hop.edgeAuthorityRank, 0) / hops.length;

    const averageConfidence =
      hops.reduce((sum, hop) => sum + hop.confidence, 0) / hops.length;
    ...
    const totalScore =
      evidenceScore + authorityScore + confidenceScore - pathLengthPenalty;
  ...
  chooseBest(paths: ScoredPath[]): ScoredPath | null {
    if (paths.length === 0) {
      return null;
    }
